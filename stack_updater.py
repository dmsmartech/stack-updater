...
# FILE PATHS
# =============================================================================
CONFIG_FILE   = "/usr/local/bin/stack_updater_config.json"
LANG_FILE     = "/usr/local/bin/stack_updater_lang.json"
LANG_FILE_IT  = "/usr/local/bin/stack_updater_lang_it.json"
LANG_FILE_EN  = "/usr/local/bin/stack_updater_lang_en.json"
...
def load_lang() -> dict:
    try:
        ...
        log.warning("Lingua non caricata: %s", e)
        return {}
...
async def switch_lang(lang_code: str) -> bool:
    """Copia il file lingua corretto su LANG_FILE e ricarica L.
    Se il file per-lingua non esiste lo scarica dal repo."""
    src = LANG_FILE_IT if lang_code == "it" else LANG_FILE_EN
    if not Path(src).exists():
        # Salva il file corrente sotto il nome della lingua corrente (se ancora non c'è)
        current = LANG_FILE_IT if cfg_lang() == "it" else LANG_FILE_EN
        if not Path(current).exists() and Path(LANG_FILE).exists():
            shutil.copy2(LANG_FILE, current)
        # Scarica il file della lingua richiesta
        code, _ = await run_cmd([
            "curl", "-fsSL",
            f"{REPO_BASE}/languages/{lang_code}.json",
            "-o", src,
        ])
        if code != 0 or not Path(src).exists():
            return False
    shutil.copy2(src, LANG_FILE)
    c = load_config()
    c["lang"] = lang_code
    save_config(c)
    L.clear()
...
async def get_containers() -> list[dict]:
    """Ritorna lista di container con service name, container name, immagine, tag, stato."""
    _, out = await run_cmd([
        "docker", "compose", "ps",
...
async def updates_menu_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "nav:main":
        name = cfg_name()
        await edit(update, t("main_menu", name=name), kb(
            InlineKeyboardButton(t("btn_updates"),  callback_data="nav:updates"),
            InlineKeyboardButton(t("btn_settings"), callback_data="nav:settings"),
        ))
        return MAIN_MENU
...
async def _show_containers_list(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not containers:
        ...
        return
    btns = [InlineKeyboardButton(f"🐳 {c['name']}", callback_data=f"ct:detail:{c['service']}")
            for c in containers]
...
async def _show_settings(update: Update):
    await edit(update, t("settings_menu"), kb(
...
async def settings_input_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if key == "dir":
        ...
        c["docker_dir"] = text
        save_config(c)
        await update.message.reply_html(t("settings_saved_dir", value=text), reply_markup=settings_kb)
        ...
        await update.message.reply_html(t("settings_saved_day", value=text), reply_markup=settings_kb)
        ...
        save_config(c)
        _reschedule_reminder(ctx)
        await update.message.reply_html(t("settings_saved_time", value=text), reply_markup=settings_kb)
        ...
        c["user_name"] = text
        save_config(c)
        await update.message.reply_html(t("settings_saved_username", value=text), reply_markup=settings_kb)
        ...
        await update.message.reply_html(t("settings_saved_lang", value=lang_label), reply_markup=new_settings_kb)
